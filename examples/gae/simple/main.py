...
class Login(webapp2.RequestHandler):
    ...
    def any(self, provider_name):
        ...
        if result.error:
            ...
            self.response.write(f'<h2>Damn that error: {result.error.message}</h2>')
            ...
            self.response.write(f'<h2>Your id is: {result.user.id}</h2>')
            self.response.write(f'<h2>Your email is: {result.user.email}</h2>')
                ...
                if result.provider.name == 'fb':
                    ...
                    if response.status == 200:
                        ...
                        if error := response.data.get('error'):
                            self.response.write(f'Damn that error: {error}!')
                        elif statuses:
                            ...
                            for message in statuses:
                                ...
                                self.response.write(f'<h3>{text}</h3>')
                                self.response.write(f'Posted on: {date}')
                    else:
                        self.response.write('Damn that unknown error!<br />')
                        self.response.write(f'Status: {response.status}')
                    ...
                    if response.status == 200:
                        if type(response.data) is list:
                            ...
                            for tweet in response.data:
                                ...
                                self.response.write(f'Tweeted on: {date}')

                        elif response.data.get('errors'):
                            self.response.write(f"Damn that error: {response.data.get('errors')}!")
                    else:
                        self.response.write('Damn that unknown error!<br />')
                        self.response.write(f'Status: {response.status}')
