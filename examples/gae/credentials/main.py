...
class Home(webapp2.RequestHandler):
    def get(self):
        ...
        if error := urllib.unquote(self.request.cookies.get('error', '')):
...
class Refresh(webapp2.RequestHandler):
    def get(self):
        ...
        if response := credentials.refresh(force=True):
    ...
    def post(self, provider_name):
        ...
        if provider_name == 'fb':
            ...
            if error := response.data.get('error'):
                ...
                self.response.write(
                    f'<p>You just posted a status with id {post_id} to your Facebook timeline.<p/>'
                )
            else:
                self.response.write(
